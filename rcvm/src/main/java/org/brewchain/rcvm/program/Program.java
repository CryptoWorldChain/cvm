...
import static org.brewchain.rcvm.utils.BIUtil.isNotCovers;
import static org.brewchain.rcvm.utils.BIUtil.isPositive;
import static org.brewchain.rcvm.utils.BIUtil.toBI;
...
import org.brewchain.ecrypto.address.Address;
import org.brewchain.evm.api.EvmApi;
import org.brewchain.evmapi.gens.Act.Account;
import org.brewchain.rcvm.base.DataWord;
import org.brewchain.rcvm.config.BlockchainConfig;
import org.brewchain.rcvm.exec.MessageCall;
import org.brewchain.rcvm.exec.OpCode;
import org.brewchain.rcvm.exec.PrecompiledContracts.PrecompiledContract;
import org.brewchain.rcvm.exec.VM;
import org.brewchain.rcvm.exec.invoke.ProgramInvoke;
import org.brewchain.rcvm.exec.invoke.ProgramInvokeFactory;
import org.brewchain.rcvm.exec.invoke.ProgramInvokeFactoryImpl;
import org.brewchain.rcvm.exec.listener.CompositeProgramListener;
import org.brewchain.rcvm.exec.listener.ProgramListenerAware;
import org.brewchain.rcvm.exec.listener.ProgramStorageChangeListener;
...
public class Program {
	...
	public Program(byte[] codeHash, byte[] ops, ProgramInvoke programInvoke, MultiTransaction transaction) {
		...
		// config.getBlockchainConfig().getConfigForBlock(programInvoke.getNumber().longValue());
	}
	...
	// private InternalTransaction addInternalTx(byte[] nonce, DataWord gasLimit,
	// byte[] senderAddress, byte[] receiveAddress,
	// BigInteger value, byte[] data, String note) {
	//
	// InternalTransaction result = null;
	// if (transaction != null) {
	// byte[] senderNonce = isEmpty(nonce) ?
	// ByteUtil.intToBytes((getStorage().getNonce(senderAddress))) : nonce;
	//
	// data = null;
	// result =
	// getResult().addInternalTransaction(transaction.getTxHash().toByteArray(),
	// getCallDeep(), senderNonce,
	// getGasPrice(), gasLimit, senderAddress, receiveAddress, value.toByteArray(),
	// data, note);
	// }
	//
	// return result;
	...
	public void callToAddress(MessageCall msg) {
		...
		// FETCH THE CODE
		byte[] programCode = getStorage().isExist(codeAddressBS)
				? getStorage().GetAccount(codeAddressBS).getValue().getCode().toByteArray()
				: EMPTY_BYTE_ARRAY;
		...
		// InternalTransaction internalTx = addInternalTx(null, getGasLimit(),
		// senderAddress, contextAddress, endowment, data, "call");
		...
		if (isNotEmpty(programCode)) {
			...
			VM vm = new VM();
			Program program = new Program(getStorage().GetAccount(codeAddressBS).getValue().getCodeHash().toByteArray(),
			...
			// } else if (Arrays.equals(transaction.getReceiveAddress(),
			// internalTx.getReceiveAddress())) {
	...
	public void storageSave(byte[] key, byte[] val) {
		getStorage().saveStorage(ByteString.copyFrom(getOwnerAddress().getNoLeadZeroesData()), key, val);
	}
	...
	public byte[] getCodeAt(DataWord address) {
		byte[] code = getStorage().GetAccount(ByteString.copyFrom(address.getLast20Bytes())).getValue().getCode().toByteArray();// invoke.getRepository().getCode(address.getLast20Bytes());
...
//	 }
	...
	public DataWord storageLoad(DataWord key) {
		DataWord ret = new DataWord(getStorage().getStorage(ByteString.copyFrom(getOwnerAddress().getNoLeadZeroesData()), key.getData()));
	...
	//
	// if (pc != 0)
	// globalOutput.append("[Op:
	// ").append(OpCode.code(lastOp).name()).append("]\n");
	//
	...
	public void callToPrecompiledAddress(MessageCall msg, PrecompiledContract contract) {
		...
		// logger.debug("Call {}(data = {})", contract.getClass().getSimpleName(),
