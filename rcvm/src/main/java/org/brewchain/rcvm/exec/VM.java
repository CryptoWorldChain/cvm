...
public class VM {
	...
	// private long calcMemGas(long oldMemSize, BigInteger newMemSize, long
	// copySize) {
	//// long gasCost = 0;
	//
	// // Avoid overflows
	...
	// //TODO #POC9 c_quadCoeffDiv = 512, this should be a constant, not magic
	// number
	//// long memGas = ( gasCosts.getMEMORY() * memWords + memWords * memWords /
	// 512)
	...
	public void step(Program program) {
		...
		try {
			...
			/*
			 * DEBUG #POC9 if( op.asInt() == 96 || op.asInt() == -128 || op.asInt() == 57 ||
			 * op.asInt() == 115) { //byte alphaone = 0x63; //op = OpCode.code(alphaone);
			 * gasCost = 3; }
			 * 
			 * if( op.asInt() == -13 ) { //byte alphaone = 0x63; //op =
			 * OpCode.code(alphaone); gasCost = 0; }
			 */
			...
			case MSTORE:
				// gasCost += calcMemGas(gasCosts, oldMemSize, memNeeded(stack.peek(), new
				// DataWord(32)), 0);
				break;
			case MSTORE8:
				// gasCost += calcMemGas(gasCosts, oldMemSize, memNeeded(stack.peek(), new
			...
			case STATICCALL:
				...
				// if (op == CALL) {
				// if (blockchainConfig.eip161()) {
				// if (isDeadAccount(program, callAddressWord.getLast20Bytes()) &&
				// !value.isZero()) {
				...
				BigInteger in = memNeeded(stack.get(stack.size() - opOff), stack.get(stack.size() - opOff - 1)); // in
																													// offset+size
				BigInteger out = memNeeded(stack.get(stack.size() - opOff - 2), stack.get(stack.size() - opOff - 3)); // out
																														// offset+size
			...
			case LOG4:
				...
				// throw Program.Exception.notEnoughOpGas(op, dataCost,
				...
				// calcMemGas(gasCosts, oldMemSize, memNeeded(stack.peek(),
				// stack.get(stack.size() - 2)), 0);
				break;
			case EXP:
			...
			// DEBUG System.out.println(" OP IS " + op.name() + " GASCOST IS " + gasCost + "
			// NUM IS " + op.asInt());
			...
			case BLOCKHASH: {
				 ...
				 * 
				 * if (log.isInfoEnabled()) hint = "blockHash: " + blockHash;
				 * 
				 * program.stackPush(blockHash); program.step();
				 */
			}
				break;
			case COINBASE: {
			...
			case STATICCALL: {
				...
				hint = "addr: " + Hex.toHexString(codeAddress.getLast20Bytes()) + " gas: " + 0 + " inOff: "
				...
				PrecompiledContracts.PrecompiledContract contract = PrecompiledContracts
						.getContractForAddress(codeAddress, null);
			...
			case REVERT: {
				...
				if (log.isInfoEnabled())
					hint = "data: " + Hex.toHexString(hReturn) + " offset: " + offset.value() + " size: "
							+ size.value();
	...
	/*
	 * Dumping the VM state at the current operation in various styles - standard
	 * Not Yet Implemented - standard+ (owner address, program counter, operation,
	 * gas left) - pretty (stack, memory, storage, level, contract, vmCounter,
	 * internalSteps, operation gasBefore, gasCost, memWords)
	 */
	// private void dumpLine(OpCode op, long gasBefore, long gasCost, long memWords,
	// Program program) {
	...
	// Hex.toHexString(program.getOwnerAddress().getLast20Bytes());
	// String pcString = Hex.toHexString(new
	// DataWord(program.getPC()).getNoLeadZeroesData());
	// String opString = Hex.toHexString(new byte[]{op.val()});
	// String gasString = Hex.toHexString(program.getGas().getNoLeadZeroesData());

	// dumpLogger.trace("{} {} {} {}", addressString, pcString, opString,
	// gasString);
	// } else
	// if (config.dumpStyle().equals("pretty")) {
	...
	// String contract =
	// Hex.toHexString(program.getOwnerAddress().getLast20Bytes());
	// String internalSteps = String.format("%4s",
	// Integer.toHexString(program.getPC())).replace(' ', '0').toUpperCase();
